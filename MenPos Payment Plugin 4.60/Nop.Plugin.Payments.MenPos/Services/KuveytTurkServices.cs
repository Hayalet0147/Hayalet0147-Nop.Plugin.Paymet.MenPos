...
namespace Nop.Plugin.Payments.MenPos.Services
{
    public class KuveytTurkServices
    {
        ...
        public string GetDataAsXml(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            var okUrl = $"{_webHelper.GetStoreLocation()}Plugins/PaymentMenPos/SendApprove";
            //Success url (Redirect to this url if success 3D code)
            var failUrl = $"{_webHelper.GetStoreLocation()}Plugins/PaymentMenPos/Fail";
            //Name of card holder
        ...
        public string PutHtmlCodeInFile(string result)
        {
            ...
            var file = "tmp" + DateTime.UtcNow.Ticks + "{" + _workContext.GetCurrentCustomerAsync().Id + "}.html";
        ...
        public async Task<string> GetErrorMessage(string responseCode)
        {
            string response;
